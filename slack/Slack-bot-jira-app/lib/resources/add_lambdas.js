...
const add_direct_jira_creator_lambda = (scope, role, layer) => {

  const name = NAME.SLACK_BOT_DIRECT_JIRA_CREATOR_NAME;
  return new lambda.Function(scope, name, {
    runtime: lambda.Runtime.NODEJS_12_X,
    functionName: name,
    description: name,
    timeout: cdk.Duration.minutes(4),
    role: role,
    code: lambda.Code.asset('./lambdas/slack_bot_direct_jira_creator'),
    layers: [layer],
    memorySize: 256,
    handler: 'app.lambdaHandler'
  });
...
module.exports = (scope) => {
  ...
  const direct_jira_lambda = add_direct_jira_creator_lambda(scope, role, layer);
  
  return {
    sqs_publisher: sqs_publisher,
    sqs_consumer: sqs_consumer,
    direct_jira_lambda: direct_jira_lambda
  };
};